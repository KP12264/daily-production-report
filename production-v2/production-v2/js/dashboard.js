...
function render(){
  ...
  let plan=pb.reduce((a,b)=>a+b,0), actual=ab.reduce((a,b)=>a+b,0);if(!$("dashModel").value&&!$("dashDoor").value){if(!plan)plan=Number(S.plan?.adjustedPlan??S.plan?.totalPlan??0);let savedTotal=Number(S.actual?.actualTotal);if(savedTotal&&!actual)actual=savedTotal;}
