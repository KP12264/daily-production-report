...
function rowsFromPlan(){
 let out={}, ms=S.plan?.masterSnapshot||{};
 // Step 5/6 saved matrix: each time block has cells/rows with model, door, plan.
 let sources=[S.plan?.matrix,ms.matrix,S.plan?.planMatrix,ms.planMatrix,S.plan?.planRows,ms.planRows].filter(Array.isArray);
 for(const src of sources){
   src.forEach((block,bi)=>{
     // block may itself be a row-oriented record or a time-block record.
     let cells=block?.cells||block?.rows||block?.models||block?.values||block?.qtyByModel||block?.planByModel;
     if(Array.isArray(cells)){
       cells.forEach(c=>{
         let model=c.model||c.modelName,door=c.door||c.doorName;
         if(!model)return;
         let kk=k(model,door), idx=Number(c.blockIndex??bi);
         (out[kk]??=[])[idx]=Number(c.adjustedPlan??c.plan??c.qty??c.value??0);
       });
     }else if(cells && typeof cells==="object"){
       Object.entries(cells).forEach(([kk,v])=>{
         (out[kk]??=[])[bi]=Number(typeof v==="object"?(v.adjustedPlan??v.plan??v.qty??v.value??0):v);
       });
     }else if(block?.model||block?.modelName){
       let kk=k(block.model||block.modelName,block.door||block.doorName);
       let vals=block.blocks||block.values||block.qtyByBlock||block.planByBlock;
       if(Array.isArray(vals))out[kk]=vals.map(v=>Number(typeof v==="object"?(v.adjustedPlan??v.plan??v.qty??v.value??0):v));
     }
   });
...
function actualRows(){
 let out={},a=S.actual?.actualByCell||S.actual?.cells||{};
...
function fallbackPlanRows(){
 let out={}, snap=S.actual?.planSnapshot||{};
 let src=S.actual?.planByCell||snap.planByCell||{};
 Object.entries(src).forEach(([cell,v])=>{let p=cell.split("|||"),bi=Number(p[0]),kk=k(p[1],p[2]);(out[kk]??=[])[bi]=Number(typeof v==="object"?(v.adjustedPlan??v.plan??v.qty??0):v)});
 // Some Entry versions store plan rows directly.
 let rows=S.actual?.planRows||snap.planRows||snap.rows||[];
 if(!Object.keys(out).length&&Array.isArray(rows))rows.forEach(r=>{let kk=k(r.model||r.modelName,r.door||r.doorName);out[kk]=(r.blocks||r.values||r.qtyByBlock||[]).map(v=>Number(typeof v==="object"?(v.adjustedPlan??v.plan??v.qty??0):v))});
 return out;
}
