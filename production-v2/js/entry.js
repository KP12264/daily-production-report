...
let S={lines:[],plan:null,actual:{},docId:null,saveTimers:new Map()};
...
async function queueSave(k,v){
 clearTimeout(S.saveTimers.get(k));S.saveTimers.set(k,setTimeout(async()=>{
   try{
    let payload={date:$("entryDate").value,lineId:$("entryLine").value.toUpperCase(),shift:$("entryShift").value,planId:S.plan.id||`plan_${$("entryDate").value}_${$("entryLine").value.toUpperCase()}_${$("entryShift").value}`,actualByCell:{[k]:v},updatedAt:firebase.firestore.FieldValue.serverTimestamp(),version:1};
